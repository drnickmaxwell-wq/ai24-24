...
let CoastalParticles:any; try{CoastalParticles=require('@/components/effects/CoastalParticles').default}catch{CoastalParticles=({density='medium'}:{density?:'low'|'medium'|'high'})=>{const c=density==='high'?70:density==='low'?25:45;return <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden">{Array.from({length:c}).map((_,i)=>{const size=Math.random()*3+1,left=Math.random()*100,top=Math.random()*100,dur=8+Math.random()*12,delay=Math.random()*6;return <span key={i} style={{position:'absolute',left:`${left}%`,top:`${top}%`,width:size,height:size,borderRadius:'50%',background:'rgba(255,255,255,.45)',boxShadow:'0 0 12px rgba(255,255,255,.35)',animation:`floatParticle ${dur}s ease-in-out ${delay}s infinite`}}/>})}<style jsx>{`@keyframes floatParticle{0%{transform:translateY(0) translateX(0);opacity:.7}50%{transform:translateY(-18px) translateX(6px);opacity:.9}100%{transform:translateY(0) translateX(0);opacity:.7}`}</style></div>;}}
export default function HeroLuxe(){
  return (<section className="relative overflow-hidden">
    <div className="absolute inset-0 pointer-events-none"><WaveBackground amplitude={0.6} speed={0.25}/><CoastalParticles density="medium"/></div>
    <div className="relative z-10">
      <video className="w-full h-[70vh] object-cover" poster="/videos/hero/hero-poster.jpg" autoPlay muted playsInline loop>
        <source src="/videos/hero/hero.webm" type="video/webm"/><source src="/videos/hero/hero.mp4" type="video/mp4"/>
      </video>
      <div className="absolute inset-0 flex items-center"><div className="mx-auto w-full max-w-[1200px] px-6"><div className="max-w-3xl">
