    ...
    <section className="relative py-16">
      <div className="mx-auto w-full max-w-[1200px] px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {features.map((f) => (
          <div
            key={f.title}
            className="p-6 rounded-xl bg-white/80 backdrop-blur-md border border-white/30 shadow-lg hover:shadow-xl transition"
            style={{ boxShadow: 'var(--shadow-elevated)' }}
          >
            <h3 className="text-lg font-semibold">{f.title}</h3>
            <p className="mt-2 text-sm text-[var(--text)]/70">{f.text}</p>
          </div>
        ))}
      </div>
    </section>
